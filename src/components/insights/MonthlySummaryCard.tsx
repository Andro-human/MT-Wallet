...
export function MonthlySummaryCard({
  ...
  month: string | null;
  buildAggregates: () => MonthlyAggregates;
}) {
  ...
  const run = async () => {
    try {
      await generate.mutateAsync(buildAggregates());
    } catch (e) {
    ...
    >
      <div className="flex items-center justify-between mb-3 gap-3">
        <h3 className="font-heading font-bold text-foreground flex items-center gap-2">
          <Sparkles className="w-4 h-4" /> {format(new Date(`${month}-01`), 'MMMM')} in review
        </h3>
        <button
          onClick={run}
      ...
      {cached ? (
        ...
        <p className="text-sm text-muted-foreground">
          {generate.isPending
            ? 'Writing your month...'
            : 'A short AI recap of where the money went this month.'}
        </p>
      )}
    </motion.div>
  );
